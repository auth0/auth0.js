...
/**
 * _shouldAuthenticateWithCordovaPlugin method checks whether Auth0 is properly configured to handle authentication
 * of a social connnection using a phonegap plugin.
 *
...
Auth0.prototype._socialPhonegapLogin = function(options, callback) {
  ...
  var self = this;
  socialAuthentication(options.connection_scope, function(accessToken, extras, error) {
    if (error) {
 ...
 * @param {Object}   scopes     FB scopes used to login. It can be an Array of String or a single String. By default is ["public_profile"]
 * @param {function} callback   To be called after login happened (whether
 *                              success or failure). It will yield the accessToken and any extra information neeeded by Auth0 API
 *                              or an Error if the authentication fails. 
 *                              Callback should be:
 *                              function (accessToken, extras, error)
 * @private
 */
...
Auth0.prototype._phonegapFacebookLogin = function(scopes, callback) {
  ...
  facebookConnectPlugin.login(fbScopes, function (state) {
    callback(state.authResponse.accessToken, {}, null);
  }, function(error) {
    callback(null, null, new Error(error));
  }); 
};
