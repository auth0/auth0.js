...
Auth0.prototype.loginPhonegap = function (options, callback) {
  if (options.connection === "facebook") {
    var self = this;
    facebookConnectPlugin.login(["email"], function (state) {
      console.log(state);
      var loginOptions = xtend({ access_token: state.authResponse.accessToken }, options);
      self.loginWithSocialAccessToken(loginOptions, callback);
    }, function(error) {
      callback(new Error(error), null, null, null, null);
    });
    return;
  }
